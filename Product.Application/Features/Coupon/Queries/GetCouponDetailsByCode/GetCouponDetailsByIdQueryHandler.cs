...
{
    internal class GetCouponDetailsByIdQueryHandler : IQueryHandler<GetCouponDetailsByIdQuery, CouponDto>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;
        private readonly IUserManagement _userManagement;
        public GetCouponDetailsByIdQueryHandler(IUnitOfWork unitOfWork, IMapper mapper, IUserManagement userManagement)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
            _userManagement = userManagement;
        }
        public async Task<ResponseModel<CouponDto>> Handle(GetCouponDetailsByIdQuery request, CancellationToken cancellationToken)
        {
            var coupon = _unitOfWork.Repository<Domain.Entities.Coupon>().GetEntityWithSpec(new GetCouponsByStatusAndDateSpecification(request.Code));
            if (coupon == null)
